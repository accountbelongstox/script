...
MAIN_DIR=$(sudo "$python_interpreter" "$main_script" env get_val "MAIN_DIR")
WEB_DIR=$(sudo "$python_interpreter" "$main_script" env get_val "WEB_DIR")
BT_IMAGE=$(sudo "$python_interpreter" "$main_script" env get_val "BT_IMAGE")
DOCKER_DATA=$(sudo "$python_interpreter" "$main_script" env get_val "DOCKER_DATA")
SERVICE_DIR=$(sudo "$python_interpreter" "$main_script" env get_val "SERVICE_DIR")
BT_USER=$(sudo "$python_interpreter" "$main_script" env get_val "BT_USER")
BT_PWD=$(sudo "$python_interpreter" "$main_script" env get_val "BT_PWD")
BT_ENTRY=$(sudo "$python_interpreter" "$main_script" env get_val "BT_ENTRY")
POSTGRES_USER=$(sudo "$python_interpreter" "$main_script" env get_val "POSTGRES_USER")
POSTGRES_PASSWORD=$(sudo "$python_interpreter" "$main_script" env get_val "POSTGRES_PASSWORD")
SAMBA_USER=$(sudo "$python_interpreter" "$main_script" env get_val "SAMBA_USER")
SAMBA_PWD=$(sudo "$python_interpreter" "$main_script" env get_val "SAMBA_PWD")
MYSQL_ROOT_USER=$(sudo "$python_interpreter" "$main_script" env get_val "MYSQL_ROOT_USER")
MYSQL_ROOT_PASSWORD=$(sudo "$python_interpreter" "$main_script" env get_val "MYSQL_ROOT_PASSWORD")
MYSQL_USER=$(sudo "$python_interpreter" "$main_script" env get_val "MYSQL_USER")
MYSQL_PASSWORD=$(sudo "$python_interpreter" "$main_script" env get_val "MYSQL_PASSWORD")
ZEROTIER_DOMIAN=$(sudo "$python_interpreter" "$main_script" env get_val "ZEROTIER_DOMIAN")
ZTNCUI_PASSWD=$(sudo "$python_interpreter" "$main_script" env get_val "ZTNCUI_PASSWD")
echo "DOCKER_DATA ${DOCKER_DATA}..."
echo "SERVICE_DIR ${SERVICE_DIR}..."
selected_services=$(sudo "$python_interpreter" "$main_script" env get_val "docker_compose")
...
sudo docker -f "$compose_file" up -d
if [ $? -ne 0 ]; then
    echo "Error: docker-compose up failed. Trying $compose_file config..."
    sudo docker -f "$compose_file" config
    exit 1
fi
for service in "${services[@]}"; do
    if [[ "$service" == *"nginx-proxy-manager"* ]]; then
        echo "nginx-proxy-manager. Installing required packages..."
        sudo docker exec -it nginx-proxy-manager "$python_interpreter" -m pip show zope > /dev/null 2>&1
        if [ $? -eq 0 ]; then
