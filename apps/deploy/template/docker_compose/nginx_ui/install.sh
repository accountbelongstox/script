...
# export DATA_PATH='/usr/local/etc/nginx_ui'
DataPath=${DATA_PATH:-/usr/local/etc/nginx_ui}
...
install_service() {
    mkdir -p '/etc/systemd/system/nginx_ui.service.d'
cat > "$ServicePath" << EOF
[Unit]
...
start_nginx_ui() {
    if [[ -f "$ServicePath" ]]; then
        ...
        sleep 1s
        if systemctl -q is-active nginx_ui; then
...
stop_nginx_ui() {
    if ! systemctl stop nginx_ui; then
...
remove_nginx_ui() {
  if systemctl list-unit-files | grep -qw 'nginx_ui'; then
    if [[ -n "$(pidof nginx-ui)" ]]; then
      stop_nginx_ui
    fi
    local delete_files=('/usr/local/bin/nginx_ui' '/etc/systemd/system/nginx_ui.service' '/etc/systemd/system/nginx_ui.service.d')
    if [[ "$PURGE" -eq '1' ]]; then
        [[ -d "$DataPath" ]] && delete_files+=("$DataPath")
    fi
    systemctl disable nginx_ui
...
main() {
    ...
    # Determine if nginx_ui is running
    if systemctl list-unit-files | grep -qw 'nginx_ui'; then
        if [[ -n "$(pidof nginx-ui)" ]]; then
            stop_nginx_ui
            NGINX_UI_RUNNING='1'
    ...
    install_bin
    echo 'installed: /usr/local/bin/nginx_ui'
