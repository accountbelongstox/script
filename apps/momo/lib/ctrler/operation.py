...
class Operation(Base):
    ...
    def test(self,project_name):
        ...
        self.icons_name = os.path.join(project_name,"icons")
        a_position = cv2cognize.find_operate_position_one("a",self.icons_name,matching_index=3)
        oper.click_center(a_position)
        self.info("切换到首页")
        # self.info(a_position)
        time.sleep(5)
        b_position = cv2cognize.find_operate_position_one("b",self.icons_name,False,matching_index=3)
        self.info("切换到小宇宙")
        # self.info(b_position)
        oper.click_center(b_position)
        self.info("识别信息调用中...")
        time.sleep(2)
