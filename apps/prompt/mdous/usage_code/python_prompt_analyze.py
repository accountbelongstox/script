...
class pythonUsageCode(PromptAnalyzeInterface,Base):
    ...
    def split_comments(self, text_raw_arr):
        code_arr = []
        current_line = ""
        for raw_text in text_raw_arr:
            words = raw_text.split()
            for word in words:
                if len(current_line) + len(word) + 1 <= self.max_line:
                    current_line += word + " "
                else:
                    code_arr.append(current_line.strip())
                    current_line = word + " "
