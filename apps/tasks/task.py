...
class TaskManager(Base):
                ...
                if tasks:
                    ...
                    for key, item in tasks.items():
                        if key not in self.task_list:
                            ...
                            new_task_count += 1
                            self.success(f"Get a tasks as {key}")
    ...
    def save_done_to_cache(self, filename, content,task_entity=None):
        ...
        self.success(f"Save the tasks-complete. Content length:{len(strtool.to_str(content))}. direct : {new_filename}.")
