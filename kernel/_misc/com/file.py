...
class File(Base):
    ...
    def save_file(self, file_name=None, content=None, encoding=None, overwrite=False):
        ...
        self.com_util.print_info(f"save file ({encoding}) to \"{file_name}\"")
    ...
    def cut(self, src, dst):
        if self.is_file(src) and self.is_file(dst):
            # self.com_util.print_warn(f"file cut error: file exists {dst}")
            os.remove(src)
            # os.remove(dst)
            return
        if self.is_file(src):
            ...
            self.com_util.print_warn(f"file cut error(src no exists):\n src:{src}\n dst:{dst}")

    def wait_cut(self, src, dst, wait=0):
        if wait > 30:
            self.com_util.print_warn(f"down file {src} fail, file cut.")
        else:
            wait += 1
    ...
    def remove_old_subdirs(self, dir_path, pre_fix="", out_time=None, not_include=None, include=None):
        ...
        for subdir in os.listdir(dir_path):
            ...
            try:
                ...
                self.com_util.print_warn(f"remove_old_subdirs {e}")
