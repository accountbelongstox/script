...
class Exchange(Base):
    ...
    def main(self,args):
        self.time_difference = self.calculate_time_difference()
    ...
    def get_rate_list_from_web(self):
        if len(self.__rate_list) == 0 \
                or \
                datetime.now().strftime('%Y-%m-%d %H:%i:%S').__eq__(self.rate_list[0]["time_update"]) == True:
    ...
    def compare_last_price(self,arrADICT, arrBDICT, instId):
        if arrBDICT == None:
            return (0,0,None,None)
        timestampfrom = arrADICT.get("timestamp")
        timestampto = arrBDICT.get("timestamp")
        arrA = arrADICT.get("data")
        arrB = arrBDICT.get("data")
        timesfrom = self.com_util.timetoformat(timestampfrom)
        timesto = self.com_util.timetoformat(timestampto)
        for itemA in arrA:
            if itemA['instId'] == instId:
                for itemB in arrB:
                    if itemB['instId'] == instId:
                        a_price = float(itemA['lastPrice'])
                        diff = a_price - float(itemB['lastPrice'])
                        rate = self.com_util.calculate_percentage(diff,a_price)
                        rate = float(rate)
                        return (diff,rate,timestampto,f"{timesfrom} -> {timesto}")
        return (0,0,None,None)

    def find_nearest_value(self, arr, before_time):
        index = len(arr) - 1
        while index >= 0:
            item = arr[index]
            timestamp = item['timestamp']
            timestamp = int(timestamp)
            diff = before_time - timestamp
            if diff > 0:
                offset_diff = timestamp - diff
                if offset_diff > 5:
                    return item
                else:
                    return None
    ...
    def get_USTDS(self,data, utc_name):
        if data != None:
            data = data.get("data")
            # for item in data:
            #     if utc_name in item:
            #         return item[utc_name]
        return data

    def find_value_within(self, arr, string_time,now_timestamp):
        time_diff = self.com_util.before_timestamp_diff(string_time, now_timestamp)
        before_time = now_timestamp - (time_diff * 1000)
        closest_time = self.find_nearest_value(arr, before_time)
        return closest_time

    def find_historyraisedown_price(self,time_string,now_timestamp,raise_down_prices_history):
        find_before = self.find_value_within(raise_down_prices_history, time_string,now_timestamp)
        find_usdt = self.get_USTDS(find_before, "utc24")
        if find_usdt != None:
            find_usdt = {
                "timestamp":find_before.get("timestamp"),
                "data":find_usdt,
            }
        return find_usdt

    def serialize_processsubscribe(self,tabname,data):
        if tabname.endswith("coin_names"):
            save_data = {
                "coin_name":data
            }
            self.db_mysql.save(tabname, save_data)

    def calculate_time_difference(self):
        sync_hk_time = self.com_config.get_global('sync_hk_time')
        local_time_before_request = datetime.now()
        if sync_hk_time == True:
            response = requests.get('http://worldtimeapi.org/api/timezone/Asia/Hong_Kong', timeout=2)
            data = response.json()
            hk_unixtime = data['unixtime']
        else:
            hk_unixtime =self.com_util.create_timestamp()
        hk_time = datetime.fromtimestamp(hk_unixtime)

        local_time_after_request = datetime.now()
        request_time_difference = local_time_after_request - local_time_before_request
        adjusted_hk_time = hk_time + request_time_difference

        time_difference = local_time_after_request - adjusted_hk_time

        diff_seconds = time_difference.total_seconds()
        self.com_util.print_info(f"Exchange HK time sync. diff {diff_seconds}")
        return time_difference

    def get_hk_time(self):
        current_local_time = datetime.now()
        hk_time = current_local_time - self.time_difference
        return hk_time

    def get_HKtimestamp(self):
        hk_time = self.get_hk_time()
        hk_timestamp = int(hk_time.timestamp())
        return hk_timestamp

    def get_HKformattime(self, format="Y-m-d H:M:S"):
        hk_time = self.get_hk_time()
        ...
        if format == "Y-m-d H:M:S":
            ...
            return hk_time.strftime('%H:%M:%S')
        else:
            self.com_util.print_info("Unsupported format")
            return hk_time.strftime('%Y-%m-%d %H:%M:%S')

    def get_HKtodaytime(self, todaytime='H:M:S'):
        hk_time = self.get_hk_time()
        hk_today_time = hk_time.strftime(todaytime)
        return hk_today_time

    def HKtransformLocalReturnTimstamp(self, hk_timestamp):
        hk_time = datetime.fromtimestamp(hk_timestamp)
        local_time = hk_time + self.time_difference
        local_timestamp = int(local_time.timestamp())
        return local_timestamp

    def LocaltransformHKReturnTimstamp(self, local_timestamp):
        local_time = datetime.fromtimestamp(local_timestamp)
        hk_time = local_time - self.time_difference
        hk_timestamp = int(hk_time.timestamp())
        return hk_timestamp

    def from_resultgetavail(self,dct, target_key):
        if target_key in dct:
            return dct[target_key]

        for key, value in dct.items():
            if isinstance(value, dict):
                result = self.from_resultgetavail(value, target_key)
                if result is not None:
                    return result
            elif isinstance(value, list):
                for item in value:
                    if isinstance(item, dict):
                        result = self.from_resultgetavail(item, target_key)
                        if result is not None:
                            return result
        return 0
    # 添加数据进行分析
    ...
    def _analyze_trend(self):
        X = np.array(range(len(self.price_data))).reshape(-1, 1)
        y = np.array(self.price_data)

        model = LinearRegression().fit(X, y)
        slope = model.coef_[0]

        if self.prev_slope is not None:
            if self.prev_slope > 0 and slope < 0:
                self.uptrend_turns_down()
            elif self.prev_slope < 0 and slope > 0:
                self.downtrend_turned_up()

        self.prev_slope = slope

    def uptrend_turns_down(self):
        print("上涨趋势转为下跌")

    def downtrend_turned_up(self):
        print("下跌趋势转为上涨")

    def generate_order_id(self):
        given_order_id = self.com_util.create_timestamp()
        return "curses.pyc"+str(given_order_id)
    ...
    def parse_redisparsestring(self,input_array):
        def parse_item(item):
            item = item.split(',')
            last_price = item[0]
            timestamp = item[1]
            ...
            return {
                ...
                'timestamp': int(timestamp),
                'turnOver24h': float(turnover_24h)
            }

        result = [parse_item(item) for item in input_array]
