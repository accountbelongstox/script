...
class Strtool():
    ...
    def create_pwd(self, length=12):
        return self.create_password(length)

    def create_password(self, length=8):
    ...
    def sort(self, s, lower=False, filter=None):
        ...
        if filter != None:
            if type(filter) == str:
                ...
                filter = self.unique_list(filter)
                filter = self.deduplication_list(filter)
            for char in filter:
                s = "".join(s.split(char))
        return s
    ...
    def to_array_by_space(self, input_string):
        parts = re.split(r'\s+', input_string)
        return parts

    def to_array_by_newline(self, input_string):
        lines = re.split(r'\n+', input_string)
        return lines
